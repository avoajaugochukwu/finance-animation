...
const getLayoutBadge = (layoutType?: string) => {
  if (!layoutType) return null;
  const config: Record<string, { icon: React.ElementType; label: string; color: string }> = {
    split: { icon: SplitSquareHorizontal, label: 'Split', color: 'bg-purple-100 text-purple-700' },
    overlay: { icon: Layers, label: 'Overlay', color: 'bg-blue-100 text-blue-700' },
    ui: { icon: Smartphone, label: 'UI Mock', color: 'bg-green-100 text-green-700' },
    diagram: { icon: BarChart3, label: 'Diagram', color: 'bg-orange-100 text-orange-700' },
  };
  const layoutConfig = config[layoutType];
  if (!layoutConfig) return null;
  const { icon: Icon, label, color } = layoutConfig;
  return (
        ...
        {storyboardScenes.map((scene) => (
          ...
          >
            <CardContent className="p-4 space-y-3">
              ...
              {/* External Asset Suggestion */}
              {scene.external_asset_suggestion && (
                <div className="mt-2 p-2 bg-amber-50 rounded text-xs text-amber-700 border border-amber-200">
                  <span className="font-medium">Overlay:</span> {scene.external_asset_suggestion}
                </div>
              )}
            </CardContent>
          </Card>
